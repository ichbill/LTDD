...
import sys
sys.path.append("/data/zhaozhenghao/Efficiency/methods/DATM_LT")
...
from utils.dataset_tinyimbalance import TinyImbanlance
from utils.dataset_imagenet import ImageNetImbanlance
from utils.dataset_cifar10imbalance_testset import Cifar10Imbanlance_testset
from utils.class_aware_sampler import get_sampler
from utils.weight_balancing.mixup import cutmix
from utils.weight_balancing.MaxNorm import MaxNorm_via_PGD
from buffer.gsam import GSAM, ProportionScheduler
from utils import utils_gsam
...
class MakeSubset(torch.utils.data.Subset):
    def __init__(self, dataset, indices):
        super(MakeSubset, self).__init__(dataset, indices)
...
def reduce_dataset_random(train_set, class_num=10, num_per_class=5000):
    ...
    for class_idx in range(class_num):
        ...
        # num_to_select = int(num_per_class * rate)
        # print(num_per_class, rate, num_to_select)
...
def get_dataset(dataset, data_path, batch_size=1, subset="imagenette", args=None, baseline=False):
    ...
    if dataset == 'CIFAR10':
        ...
        if baseline:
            ...
            transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
        dst_train = Cifar10Imbanlance(transform=transforms.ToTensor(),imbanlance_rate=args.imbalance_rate, train=True,file_path=args.data_path)
        normalize = transforms.Normalize(mean=MEANS['cifar10'], std=STDS['cifar10'])
        transform_test = transforms.Compose([transforms.ToTensor(), normalize])
        dst_test = Cifar10Imbanlance(imbanlance_rate=args.imbalance_rate, train=False, transform=transform_test,file_path=args.data_path)
        class_names = dst_train.classes
        class_map = {x:x for x in range(num_classes)}

    elif dataset == 'CIFAR10_LT_testset':
        channel = 3
        im_size = (32, 32)
        num_classes = 10
        mean = [0.4914, 0.4822, 0.4465]
        ...
        if baseline:
            ...
            transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
        dst_train = Cifar10Imbanlance_testset(transform=transforms.ToTensor(),imbanlance_rate=args.imbalance_rate, train=True,file_path=args.data_path)
        normalize = transforms.Normalize(mean=MEANS['cifar10'], std=STDS['cifar10'])
        transform_test = transforms.Compose([transforms.ToTensor(), normalize])
        dst_test = Cifar10Imbanlance_testset(imbanlance_rate=args.imbalance_rate, train=False, transform=transform_test,file_path=args.data_path)
        class_names = dst_train.classes
        class_map = {x:x for x in range(num_classes)}
        ...
        dst_train = Cifar100Imbanlance(transform=transform_train,imbanlance_rate=args.imbalance_rate, train=True,file_path=args.data_path)
        dst_test = Cifar100Imbanlance(imbanlance_rate=args.imbalance_rate, train=False, transform=transform_test,file_path=args.data_path)
        class_names = dst_train.classes
        class_map = {x:x for x in range(num_classes)}
        ...
        dst_test = datasets.ImageFolder(os.path.join(data_path, "tiny-imagenet-200", "val", "images"), transform=transform)
        class_names = dst_train.classes
        class_map = {x:x for x in range(num_classes)}

    elif dataset == 'Tiny_LT':
        channel = 3
        im_size = (64, 64)
        num_classes = 200
        mean = [0.485, 0.456, 0.406]
        ...
        if args.zca:
            ...
            transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
        dst_train = TinyImbanlance(transform=transform, imbanlance_rate=args.imbalance_rate, train=True, file_path=os.path.join(data_path, "tiny-imagenet-200"))
        dst_test = datasets.ImageFolder(os.path.join(data_path, "tiny-imagenet-200", "val", "images"), transform=transform)
        class_names = dst_train.classes
        class_map = {x:x for x in range(num_classes)}
        ...
        dst_train = ImageNetImbanlance(imbanlance_rate=args.imbalance_rate, num_cls=num_classes, file_path=os.path.join(data_path,"imagenet/images/train"), train=True, transform=transform)
        # dst_train = ImageNetImbanlance(imbanlance_rate=args.imbalance_rate, num_cls=num_classes, file_path=data_path, train=True, transform=transform)
        dst_test = datasets.ImageFolder(os.path.join(data_path, "imagenet/images/val"), transform=transform)
        class_names = dst_train.classes
        # breakpoint()
        class_map = {x: i for i, x in enumerate(class_names)}
    ...
    if args.align_with_long_tail:
        print("Aligning with long tail, only use for baseline")
        if args.dataset == "CIFAR10":
            imbalance_dataset = Cifar10Imbanlance(imbanlance_rate=args.imbalance_rate, num_cls=10, file_path=args.data_path, train=True, transform=None, label_align=True)
        elif args.dataset == "CIFAR100":
            imbalance_dataset = Cifar100Imbanlance(imbanlance_rate=args.imbalance_rate, num_cls=100, file_path=args.data_path, train=True, transform=None, label_align=True)
        else:
            exit('unknown dataset: %s'%args.dataset)
        num_samples = sum(imbalance_dataset.get_per_class_num())
        print(f"Total number of samples: {num_samples}")
        indices = random.sample(range(len(dst_train)), num_samples)
        dst_train = MakeSubset(dst_train, indices)

        class_counts = Counter(dst_train.targets)
...
def epoch(mode, dataloader, net, optimizer, criterion, args, num_classes, aug, texture=False, If_Float = False, hard_label=False, train_criterion=None, Preciser_Scheduler=False, type=1):
    ...
    for i_batch, datum in enumerate(dataloader):
        ...
        # cutmix, only for the first stage expert
        r = np.random.rand(1)
        if args.first_stage_expert and mode == 'train':
            if args.first_stage_cutmix and r < args.first_stage_cutmix_prob:
                output, loss = cutmix(args, img, lab, net, criterion, num_classes)
            else:
                output = net(img)
        ...
        if mode == 'test':
            for i in range(n_b):
                actual_class = lab.cpu().data.numpy()[i]
                predicted_class = np.argmax(output.cpu().data.numpy(), axis=-1)[i]
    ...
    if mode == 'test':
        ...
        #     print('Accuracy of %5s : %2d %%' % (i, 100 * class_acc[i]))
    else:
        class_acc = []

    return loss_avg, acc_avg, class_acc

def epoch_f1(mode, dataloader, net, optimizer, criterion, args, num_classes, aug, texture=False, If_Float = False, hard_label=False, train_criterion=None, Preciser_Scheduler=False, type=1):
    loss_avg, acc_avg, num_exp = 0, 0, 0
    ...
    for i_batch, datum in enumerate(dataloader):
        ...
        # classwise accuracy and other metrics
        if mode == 'test':
    ...
    return loss_avg, acc_avg, class_acc, precision, recall, f1

def evaluate_synset(it_eval, net, images_train, labels_train, testloader, args, num_classes, Epoch, return_loss=False, texture=False, train_criterion=None, Preciser_Scheduler=False, type=1):
    if args.parall_eva==False:
        device = torch.device("cuda:0")
    else:
        device = args.device
    ...
    print('train for %d epochs' % Epoch)
    ...
    for ep in tqdm.tqdm(range(Epoch+1)):
        loss_train, acc_train, _ = epoch('train', trainloader, net, optimizer, train_criterion, args, num_classes, aug=True, texture=texture, If_Float=If_Float)
        acc_train_list.append(acc_train)
        loss_train_list.append(loss_train)
        if ep == Epoch:
            with torch.no_grad():
                loss_test, acc_test, class_acc = epoch('test', testloader, net, optimizer, test_criterion, args, num_classes, aug=False, If_Float = False)
        if Preciser_Scheduler:
            scheduler.step()
        else:
    ...
    # del testloader
    if return_loss:
        return net, acc_train_list, acc_test, class_acc, loss_train_list, loss_test
    else:
        return net, acc_train_list, acc_test, class_acc

def evaluate_synset_f1(it_eval, net, images_train, labels_train, testloader, args, num_classes, Epoch, return_loss=False, texture=False, train_criterion=None, Preciser_Scheduler=False, type=1):
    if args.parall_eva==False:
        device = torch.device("cuda:0")
    else:
        device = args.device
    ...
    for ep in tqdm.tqdm(range(Epoch+1)):
        ...
        if ep == Epoch:
            with torch.no_grad():
                loss_test, acc_test, class_acc, precision, recall, f1 = epoch_f1('test', testloader, net, optimizer, test_criterion, args, num_classes, aug=False, If_Float = False)
        if Preciser_Scheduler:
            scheduler.step()
        else:
            if ep in lr_schedule:
    ...
    if return_loss:
        ...
        return net, acc_train_list, acc_test, class_acc, precision, recall, f1

def evaluate_balance(it_eval, net, dst_train, testloader, args, num_classes, return_loss=False, texture=False, train_criterion=None, Preciser_Scheduler=False, type=1, hard_label=True):
    if args.parall_eva==False:
        device = torch.device("cuda:0")
    else:
        device = args.device
    ...
    # labels_train.to(device)
    lr = float(args.lr_net)
    Epoch = int(args.epoch_eval_second)
    ...
    unique_labels = dst_train.labels.unique()
    class_counts = {label.item(): (dst_train.labels == label).sum().item() for label in unique_labels}
    print(class_counts)

    # class counts in trainloader
    ...
    if Preciser_Scheduler:
        ...
        optimizer = torch.optim.SGD(net.parameters(), lr=lr, momentum=0.9, weight_decay=0.0005)
        if args.GSAM:
            scheduler = torch.optim.lr_scheduler.StepLR(optimizer,step_size=args.epoch_eval_train*len(trainloader),gamma=1)
            ...
            teacher_optim = GSAM(params=net.parameters(), base_optimizer=optimizer, 
                    model=net, gsam_alpha=args.alpha, rho_scheduler=rho_scheduler, adaptive=args.adaptive)

    start = time.time()
    acc_train_list = []
    loss_train_list = []

    # assert args.GSAM == (not Preciser_Scheduler)
    for ep in tqdm.tqdm(range(Epoch+1)):
        if args.GSAM:
            loss_train, acc_train = utils_gsam.epoch("train", dataloader=trainloader, net=net, optimizer=teacher_optim,
                                            criterion=train_criterion, args=args, aug=True,scheduler=scheduler)
            loss_test, acc_test = utils_gsam.epoch("test", dataloader=testloader, net=net, optimizer=None,
                                            criterion=test_criterion, args=args, aug=False, scheduler=scheduler)
            acc_train_list.append(acc_train)
            loss_train_list.append(loss_train)
            class_acc = None
            continue
        
        loss_train, acc_train, _ = epoch('train', trainloader, net, optimizer, train_criterion, args, num_classes, aug=True, texture=texture, If_Float=If_Float, hard_label=hard_label)
        acc_train_list.append(acc_train)
        loss_train_list.append(loss_train)
        if ep == Epoch:
            if args.second_stage_expert:
                if args.second_stage_maxnorm:
                    print('second stage maxnorm')
                    pgdFunc = MaxNorm_via_PGD(thresh=args.maxnorm_thresh)
                    pgdFunc.PGD(net)
            with torch.no_grad():
                loss_test, acc_test, class_acc = epoch('test', testloader, net, optimizer, test_criterion, args, num_classes, aug=False, If_Float = False)
        if Preciser_Scheduler:
...
def evaluate_baseline(it_eval, net, trainloader, testloader, args, num_classes, return_loss=False, texture=False, train_criterion=None, Preciser_Scheduler=False, type=1, hard_label=True):
    if args.parall_eva==False:
        device = torch.device("cuda:0")
    else:
        device = args.device
    ...
    if Preciser_Scheduler:
        ...
        lr_schedule = [Epoch//2+1]
        optimizer = torch.optim.SGD(net.parameters(), lr=lr, momentum=float(args.mom), weight_decay=float(args.l2))
        if args.GSAM:
            scheduler = torch.optim.lr_scheduler.StepLR(optimizer,step_size=args.epoch_eval_train*len(trainloader),gamma=1)
            rho_scheduler = ProportionScheduler(pytorch_lr_scheduler=scheduler, max_lr=lr, min_lr=lr,
                max_value=args.rho_max, min_value=args.rho_min)
