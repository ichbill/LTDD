...
def get_dataset(dataset, data_path, batch_size=1, subset="imagenette", args=None):
    ...
    if dataset == 'CIFAR10':
        ...
        std = [0.2023, 0.1994, 0.2010]
        transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
        dst_train = Cifar100Imbanlance(transform=transform,imbanlance_rate=args.imbalance_rate, train=True,file_path=data_path)
        dst_test = Cifar100Imbanlance(imbanlance_rate=args.imbalance_rate, train=False, transform=transform,file_path=data_path)
        class_names = dst_train.classes
        class_map = {x:x for x in range(num_classes)}
