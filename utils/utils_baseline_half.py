...
def DiffAugment(x, strategy='', seed = -1, param = None, half=False):
    if seed == -1:
...
def rand_scale_half(x, param):
    ...
    if param.batchmode: # batch-wise:
        theta[:] = theta[0]
    grid = F.affine_grid(theta, x.shape, align_corners=True).to(x.device).half()
    x = F.grid_sample(x, grid, align_corners=True)
    return x


def rand_rotate_half(x, param): # [-180, 180], 90: anticlockwise 90 degree
    ratio = param.ratio_rotate
    set_seed_DiffAug(param)
    theta = (torch.rand(x.shape[0]) - 0.5) * 2 * ratio / 180 * float(np.pi)
    theta = [[[torch.cos(theta[i]), torch.sin(-theta[i]), 0],
    ...
    if param.batchmode: # batch-wise:
        theta[:] = theta[0]
    grid = F.affine_grid(theta, x.shape, align_corners=True).to(x.device).half()
    # print(f'x: {x.dtype}, grid: {grid.dtype}')
    # breakpoint()
    x = F.grid_sample(x, grid, align_corners=True)
    return x


def rand_flip_half(x, param):
    prob = param.prob_flip
    set_seed_DiffAug(param)
    randf = torch.rand(x.size(0), 1, 1, 1, device=x.device)
    if param.batchmode: # batch-wise:
...
def rand_brightness_half(x, param):
    ratio = param.brightness
    set_seed_DiffAug(param)
    randb = torch.rand(x.size(0), 1, 1, 1, dtype=x.dtype, device=x.device)
    if param.batchmode:  # batch-wise:
...
def rand_saturation_half(x, param):
    ratio = param.saturation
    x_mean = x.mean(dim=1, keepdim=True)
    set_seed_DiffAug(param)
    rands = torch.rand(x.size(0), 1, 1, 1, dtype=x.dtype, device=x.device)
...
def rand_contrast_half(x, param):
    ratio = param.contrast
    x_mean = x.mean(dim=[1, 2, 3], keepdim=True)
    set_seed_DiffAug(param)
    randc = torch.rand(x.size(0), 1, 1, 1, dtype=x.dtype, device=x.device)
...
AUGMENT_FNS_half = {
    'color': [rand_brightness_half, rand_saturation_half, rand_contrast_half],
    'crop': [rand_crop_half],
    'cutout': [rand_cutout_half],
    'flip': [rand_flip_half],
    'scale': [rand_scale_half],
    'rotate': [rand_rotate_half],
}